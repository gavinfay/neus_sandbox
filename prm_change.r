...
change.prm4b <- function(path,origfile,param.name,outfile,mapfile,param.type,
                         name2=NULL,mapfile2=NULL,ogfile1="NeusGroups.csv",
                         ogfile2=NULL)
{
  ###### generic version that uses 2 maps
  # 1 for the variable names,
  # 2 for the vector length
  ...
  #Lookup table with new codes for functional groups along with 'parent' groups from Atlantis-neus v1.
  CodeRelations <- read.csv(mapfile)
  CodeRelations2 <- read.csv(mapfile2)
  oldg1 <- read.csv(ogfile1)
  oldg2 <- read.csv(ogfile2)
  ...
  old.codes.2 <- as.character(oldg2$Code)
  mapvals <- match(CodeRelations2$Parent,oldg2$Code)
  #mapvals2 <- c(mapvals,63:65)
  mapvals2 <- mapvals
  ...
  matches <- match(CodeRelations$Parent,tempmat[,1])
  match2 <- match(CodeRelations2$Parent,old.codes.2)
  #numcols <- unique(as.numeric(output$x[which(is.na(output$x)==FALSE)]))
  numcols <- nrow(CodeRelations2)
  output2 <- matrix(NA,ncol=numcols,nrow=nrow(output))
  for (irow in 1:nrow(output2))
  {
    output2[irow,] <-  
...
setwd("~/Atlantis/neus_sandbox")
path <- getwd()
mapfile2 <- "FleetMaps_v15.csv"
mapfile <- "coderelations.csv"
ogfile2 <- "NeusFisheries.csv"
ogfile1 <- "NeusGroups.csv"

origfile <- "at_harvest_neus_v15_DE.prm"
outfile <- "newtable.out"
param.name <- "flaghabitat_"
name2 <- "day"
param.type <- 2
change.prm(path,origfile,param.name,outfile,mapfile,param.type,name2)
change.prm3(path,origfile,param.name,outfile,mapfile,param.type,name2)
